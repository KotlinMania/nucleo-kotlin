# Claude Code Project Instructions — nucleo-kotlin

## Project Overview

This is **nucleo-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`nucleo`](https://crates.io/crates/nucleo).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/nucleo/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/nucleo>
