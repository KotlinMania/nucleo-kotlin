...
kotlin {
    ...
    val xcf = XCFramework("Nucleo")

    // Apple desktop
    macosArm64 {
        binaries.framework { baseName = "Nucleo"; xcf.add(this) }
    }
    ...
    swiftExport {
        moduleName = "Nucleo"
        flattenPackage = "io.github.kotlinmania.nucleo"
    }

    android {
        namespace = "io.github.kotlinmania.nucleo"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "nucleo-kotlin", version.toString())

    pom {
        name.set("nucleo-kotlin")
        description.set("Kotlin Multiplatform port of helix-editor/nucleo - plug and play high performance fuzzy matcher")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/nucleo-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/nucleo-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/nucleo-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/nucleo-kotlin.git")
        }
    }
}
    ...
    doFirst {
        ...
        val compileSources =
            if (sourceFiles.isEmpty()) {
                sentinelSource.get().asFile.apply {
                    parentFile.mkdirs()
                    writeText(
                        """
                ...
                listOf(sentinelSource.get().asFile)
            } else {
                sentinelSource.get().asFile.delete()
                sourceFiles
            }
